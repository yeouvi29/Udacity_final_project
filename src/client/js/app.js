...
function getData(event) {
  ...
  if (document.getElementById("city-image").firstChild) {
    document
      .getElementById("city-image")
      .removeChild(document.getElementById("city-image").childNodes[0]);
  }
...
const updateUI = (receivedData) => {
  ...
  const remainingDaysFromToday = Math.ceil(
    timeDiffFromToday / (1000 * 60 * 60 * 24)
  );
  const remainingDaysFromDDay = Math.ceil(
    timeDiffFromDDay / (1000 * 60 * 60 * 24)
  );
  ...
  document.getElementById("cityphoto").style.cssText =
    "max-width:100%;max-height:100%;min-width:100%;min-height:100%;display:block;";

  const imageUrl =
    "https://weatherbit.io/static/img/icons/" +
    receivedData.temperature.tempIcon +
    ".png";
  const newImage = document.createElement("img");
  newImage.src = imageUrl;
  newImage.setAttribute("class", "city-info-list-1-icon");
  document.getElementById("location").innerHTML =
    receivedData.geoData.city + ", " + receivedData.geoData.country;
  document.getElementById("city-info").childNodes[0].innerHTML =
    "City Information";
  console.log(document.getElementById("city-info").firstElementChild);
  document.getElementById("city-info").firstElementChild.innerHTML =
    "lat: " +
    receivedData.geoData.latitude +
    " & lon: " +
    receivedData.geoData.longitude;
  document.getElementById("city-info").lastElementChild.innerHTML =
    "Temp.: " + receivedData.temperature.temp + "F     ";
  ...
  if (remainingDaysFromDDay === 1) {
    document.getElementById("period").innerHTML =
      dateFormatTransform(receivedData.geoData.startDate) +
      " - " +
      dateFormatTransform(receivedData.geoData.endDate) +
      "(" +
      remainingDaysFromDDay +
      " day)";
  } else {
    document.getElementById("period").innerHTML =
      dateFormatTransform(receivedData.geoData.startDate) +
  ...
  if (remainingDaysFromToday === 1) {
    document.getElementById("count-down").innerHTML =
      remainingDaysFromToday + " day away to";
  } else if (remainingDaysFromToday === 0) {
    document.getElementById("count-down").innerHTML =
      "Only a few hours away to";
  } else {
    document.getElementById("count-down").innerHTML =
      remainingDaysFromToday + " days away to";
  }
};
